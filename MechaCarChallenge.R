...
#4. Import and read in the MecheCar_mpg.csv file as a dataframe.
meche_mpg <- read.csv(file=‘C:/Users/analu/Desktop/class_repo/MechaCar_Statistical_Analysis/MechaCar_mpg .csv’,check.names=F,stringsAsFactors = F)meche_mpg <- read.csv(file='C:/Users/analu/Desktop/class_repo/MechaCar_Statistical_Analysis/MechaCar_mpg.csv',check.names=F,stringsAsFactors = F)

#5. Perform linear regression using the lm() function
lm(mpg ~ vehicle_length + vehicle_weight + spoiler_angle + ground_clearance + AWD, data=meche_mpg)

#6. Using the summary() function, determine the p-value and the r-squared value for the linear regression model.
summary(lm(mpg ~ vehicle_length + vehicle_weight + spoiler_angle + ground_clearance + AWD, data=meche_mpg)) 

#Additional Step: eliminate the independent variables that have little impact on predicting mpg to see impact:
lm(mpg ~ vehicle_length + ground_clearance, data=meche_mpg)
summary(lm(mpg ~ vehicle_length + ground_clearance, data=meche_mpg)) 

### CHALLENGE 15: DELIVERABLE 2
#2. Import and read in the Suspension_Coil.csv file as a table
mecha_coil <- read.csv(file='C:/Users/analu/Desktop/class_repo/MechaCar_Statistical_Analysis/Suspension_Coil.csv',check.names=F,stringsAsFactors = F) 
